...
NASDAQ_DF = pd.read_csv("NYSE.csv")

NASDAQ_DF .set_index("Ticker", inplace=True)

NASDAQ_List = NASDAQ_DF.index.to_list()
...
filename = "Daily NYSE Full" + " "+ datetime.now().strftime("%I:%M%p on %B %d, %Y")+ ".csv"
...
Daily_DF.to_csv("Daily_NYSE_summary"+datetime.now().strftime("%I:%M%p on %B %d, %Y") +".csv")
